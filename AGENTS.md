...
- CI steps, which are the verification commands: `npm ci` → `npm run typecheck` → `npm run build` → `npm test` (job `build-test`), `npm run build` → `npm run package --workspace packages/extension` (job `package-vsix`, uploads the `fusen-vsix` artifact), `npm run build` → `xvfb-run -a npm run test:e2e` (job `e2e`, VS Code stable), and the same E2E tests in Cursor for Linux, fetched by `e2e/scripts/download-cursor.sh` and passed as `FUSEN_E2E_EXECUTABLE_PATH` (job `e2e-cursor`)
