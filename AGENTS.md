...
- CI steps, which are the verification commands: `npm ci` → `npm run typecheck` → `npm run build` → `npm test` (job `build-test`), `npm run build` → `npm run package --workspace packages/extension` (job `package-vsix`, uploads the `fusen-vsix` artifact), `npm run build` → `xvfb-run -a npm run test:e2e` (job `e2e`, VS Code stable), the same E2E tests in Cursor for Linux, fetched by `e2e/scripts/download-cursor.sh` and passed as `FUSEN_E2E_EXECUTABLE_PATH` (job `e2e-cursor`), and the same E2E tests in VS Code and Cursor with the `fusen-vsix` artifact installed by `--install-extension` instead of the extension under development: `npm run build -w fusen-core` → `xvfb-run -a npm run test:e2e` with the VSIX path in `FUSEN_E2E_VSIX_PATH` (jobs `e2e-vsix` and `e2e-cursor-vsix`)
