...
CI (`.github/workflows/ci.yml`) runs the same commands on every pull request, runs the E2E tests in both VS Code and Cursor, once with the extension under development and once with the packaged VSIX installed, uploads their screenshots as the `e2e-screenshots`, `e2e-cursor-screenshots`, `e2e-vsix-screenshots` and `e2e-cursor-vsix-screenshots` artifacts and the packaged extension as the `fusen-vsix` artifact.
