...
CI (`.github/workflows/ci.yml`) runs the same commands on every pull request, runs the E2E tests in both VS Code and Cursor, uploads their screenshots as the `e2e-screenshots` and `e2e-cursor-screenshots` artifacts and the packaged extension as the `fusen-vsix` artifact.
