...
| Distribution | The MCP server on npm (`fusen-mcp`) and the extension as a VSIX attached to GitHub Releases, both from `.github/workflows/release.yml` on a `v*` tag. Not published to the VS Code Marketplace or Open VSX | Decided in https://github.com/bannzai/fusen/issues/9 |
