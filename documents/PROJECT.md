...
| AI access | MCP server as a separate stdio process (`packages/mcp-server`, `node fusen-mcp.mjs`) | One line registers it in both Claude Code (`.mcp.json`) and Codex CLI, and it works without the editor running. |
...
| Distribution | The extension as a VSIX and the MCP server as one JavaScript file (`fusen-mcp.mjs`), both attached to GitHub Releases by `.github/workflows/release.yml` on a `v*` tag. Nothing is published to npm, the VS Code Marketplace or Open VSX. The VSIX bundles the extension and `fusen-core` into one file with esbuild, and `fusen-mcp.mjs` bundles the server with every dependency (`fusen-core`, `@modelcontextprotocol/sdk`, `zod`), so it loads nothing but Node built-ins. A clone's build writes the same file as `packages/mcp-server/dist/index.js` | Decided in https://github.com/bannzai/fusen/issues/9, https://github.com/bannzai/fusen/issues/6 and https://github.com/bannzai/fusen/issues/23. The VSIX ships no `node_modules` (the workspace packages are hoisted outside the extension directory), and `fusen-core` is a private workspace package, so the bundles are what make `fusen-core` available at run time. Publishing the server to npm (`npx fusen-mcp`) was rejected in #23: it needs an npm account and a publish token only to deliver one file, which a release asset delivers with `node` alone. The file is an ES module downloaded without a `package.json`, so it is named `.mjs`, which Node.js always loads as ESM (https://nodejs.org/api/packages.html). A `.js` name was rejected: Node.js would load it as ESM only through syntax detection, on by default since 22.7.0, and never under a `package.json` with `"type": "commonjs"`, so the server would depend on the Node.js version and on where it is saved. Publishing `fusen-core` as a second npm package was rejected earlier: it adds a public package, its name and its token scope only to share code inside this repository. The server bundle is ESM while `fusen-core` builds to CommonJS, so the bundle defines `require` with `createRequire` for the `require` calls of `fusen-core` |
| E2E | Playwright `_electron.launch` against a VS Code build downloaded by `@vscode/test-electron`, with `--extensionDevelopmentPath`. CI runs the same tests in Cursor too: the latest stable Linux AppImage from Cursor's download API (`https://cursor.com/api/download?platform=linux-x64&releaseTrack=stable`), extracted with `--appimage-extract` and launched through `FUSEN_E2E_EXECUTABLE_PATH`. CI also runs them in both editors with the VSIX from the `package-vsix` job installed by `--install-extension` into the test profile's `--extensions-dir` and no `--extensionDevelopmentPath` (`FUSEN_E2E_VSIX_PATH`) | Same approach as the VS Code smoke tests. The test saves screenshots that an agent reads to judge the UI. Cursor runs the same workbench and extension host, so running the tests in it replaces checking Cursor by hand (https://github.com/bannzai/fusen/issues/24). The AppImage is extracted because GitHub's Ubuntu runners have no FUSE to mount it. The development extension runs the tsc output of the source tree, not the esbuild bundle users install, so it passed while the VSIX could not start without `fusen-core`; the VSIX jobs run what users install (https://github.com/bannzai/fusen/issues/27). |
