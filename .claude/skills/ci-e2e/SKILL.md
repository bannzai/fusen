...
2. Fetch the result of the pushed commit:
   ...
   The script waits for the run to be created and finished, downloads the artifact into `./tmp/e2e-<run id>/` (skipped when that directory already has files) and prints `RUN_ID`, `RUN_URL`, `CONCLUSION`, `DIR` and one `SCREENSHOT=<path>` line per PNG. Options and exit codes are documented at the top of the script.
