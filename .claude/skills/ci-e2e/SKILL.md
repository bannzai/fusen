...
2. Fetch the result of the pushed commit:
   ...
   The script waits for the run to be created and finished, downloads the artifact into `./tmp/e2e-<run id>-<run attempt>/` (skipped when that directory already exists; it is created only after a complete download, and a re-run of the same run gets a new directory) and prints `RUN_ID`, `RUN_ATTEMPT`, `RUN_URL`, `CONCLUSION`, `DIR` and one `SCREENSHOT=<path>` line per PNG. Options and exit codes are documented at the top of the script.
